...
```{r}
bm0 <- readRDS("models/n25/bm0.rds")
bm1 <- readRDS("models/n25/bm1.rds")
bm1.1 <- readRDS("models/n25/bm1.1.rds")
bm1.2 <- readRDS("models/n25/bm1.2.rds")
bm1.3 <- readRDS("models/n25/bm1.3.rds")
## the full model
bm1.4 <- readRDS("models/n25/bm1.4.rds")
```
...
```{r}
data <- read_csv("data/n25/data.csv")
head(data)
```
...
but I like to see the posterior samples and wrangle them myself.)
...
# n25
ggsave ("figures/n25/bm1.4_fixef.jpeg")
```
...
tab_df(fix_tab,
       alternate.rows = T,
       title="Fixed effects",
       file="tables/n25/bm1.4_fixed.doc")
```

## plot the effect of condition across the different models ##
...
# n25
ggsave ("figures/n25/all_fixef.jpeg",
        width = 4, height = 4)
```
...
ggsave ("figures/n25/varypp.jpeg")
```
...
ggsave ("figures/n25/varyvalspp.jpeg")
```
...
We’ll use epred_draws() from tidybayes to plug in a hypothetical dataset and 
generate predictions, as we did with previous models. Only this time, instead 
of getting a single predicted value for each level of quota, we’ll get 4,000. 
The epred_draws() (like all tidybayes functions) returns a long tidy data frame, 
so it’s really easy to plot with ggplot:
...
ame_bm1.4 %>%
  mutate(condition = if_else(cond == -0.5, "congr", "incon"),
         condition = factor(condition, levels = c("congr", "incon"))) %>% 
  group_by(condition) %>% 
  median_hdi(.epred)
```

this above needs fixing - for some reason there are two cong conditions

plot - when the above is fixed
...
ggplot(diff_pred, aes(x = .epred, y = cond)) +
  ...
  labs(x = "Average marginal effect of congruency", 
       y = NULL, caption = "80% and 95% credible intervals shown in black") +
  theme_bw() 

ggsave ("figures/n25/diff_preds.jpeg")
```
