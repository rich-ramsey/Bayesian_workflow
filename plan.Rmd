...
rt, it is not that uncommon (depending on the specific task). The choice would need titrating to the domain in question.
...
we might expect in terms of SD. Here, I initially just picked items to be half the variation as participants, as that seems like a very common and general trend in this kind of work that items vary less than participants.

But we could still do with a ballpark estimate for SD and cor, so let's load a previous model that used a similar design and collected gaze-cueing data.
...
cueing task but with faces and gaze direction. The effect of cue was tiny, but
quite precise. The most important thing for the current purpose is getting a 
sense of SDs and cor values. formula was dv ~ 1 + cond + (1 + cond | pid) + (1 | stim))
...
adapt_delta = 0.95. This should reduce the complaints from stan and not cost us
too much in computational time so it is worth it. You could just leave it alone 
completely though for a sim and just a few divergent transitions.
...
## runs sims and keep the data and model objects in the output ##
...
write_csv(sim_dat_x2, "data/sim_dat_x2.csv")
```
...
write_csv(sim_p_x2, "data/sim_p_x2.csv")
```
...
```{r}
## data
sim_dat_x2 <- read_csv("data/sim_dat_x2.csv") %>% 
...
## parameters
sim_p_x2 <- read_csv("data/sim_p_x2.csv") %>%
  mutate(subj_n = factor(subj_n, levels = c("25", "50")),
         b1 = factor(b1, levels = c("0.25", "0.5")),
...
ok, these look sensible, as far as 2 exps can go. But they seem to be hitting the
right values that we expect as a basic sanity and code check.

## runs sims and only keep the parameter summaries ##
...
# Time difference 8 mins for 2 exp reps - imac  
# (8 mins per 2 sims x 50 = 6.67 hrs for 100 sims; 67 hrs for 1000 sims. ~4 days). 
